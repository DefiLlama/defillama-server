...
import {adaptersDataVolumes} from '../../data'
import { importVolumeAdapter } from "../../../utils/imports/importAdaptor"
import { Adapter } from "@defillama/adapors/adapters/types";
...
export default async (adapter?: string, onlyMissing: boolean | number = false) => {
    ...
    const dex = adaptersDataVolumes.find(dex => dex.volumeAdapter === (adapterName))
    const nowSTimestamp = Math.trunc((Date.now()) / 1000)
    if (dex) {
        const dexAdapter: Adapter = (await importVolumeAdapter(dex)).default
