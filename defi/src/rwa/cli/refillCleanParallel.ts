 ...
 *
 * Usage: ts-node defi/src/rwa/cli/refillCleanParallel.ts
 */
...
async function runBackfill(
  ...
  let ts = end;
  while (ts > start) {
    timestamps.push(getTimestampAtStartOfDay(ts));
    ts -= 86400;
  }

  console.log(`\n── Phase 1: Backfill (${timestamps.length} days, concurrency=${BACKFILL_CONCURRENCY}, dryRun=${DRY_RUN}) ──`);
  console.log(`  Preparing context (single Airtable fetch)...`);
  const context = await prepareAtvlContext(ids);
  console.log(`  Context ready. Starting parallel backfill...`);
    ...
    processor: async (timestamp: number) => {
      try {
        ...
        if (collectResults && result) {
          collected.push(...convertAtvlResult(timestamp, result, ids));
        }
      } catch (e) {
  ...
  console.log(`  Backfill done. Errors: ${errors.length}/${timestamps.length}`);
  return collected;
}
