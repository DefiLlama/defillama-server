...
async function handler() {
  ...
  await Promise.all(
    Object.entries(adapters).map(
      async ([protocolName, adapterPath]: [string, any]) => {
        try {
          const rawAdapter = require(adapterPath);
          const adapter =
            typeof rawAdapter.default === "function"
              ? { default: await rawAdapter.default() }
              : rawAdapter;
          ...
          const pData =
            pId && pId !== "" ? protocols.find((p) => p.id == pId) : null;
          const id = pData ? pData.parentProtocol || pData.name : protocolName;
          let name = id;
          if (pData?.parentProtocol) {
            name =
              parentProtocols.find((p) => p.id === pData.parentProtocol)
                ?.name ?? id;
          }
          const data = {
            data: chart,
