...
describe('Volumes API - Summary Options', () => {
  // Test with known options protocols
  const testProtocols = ['lyra', 'hegic', 'premia'];
  ...
  beforeAll(async () => {
    const results = await Promise.all(
      testProtocols.map((protocol) =>
        apiClient.get<VolumeSummaryResponse>(endpoints.VOLUMES.SUMMARY_OPTIONS(protocol))
      )
    );
  ...
  describe.each(testProtocols)('Protocol: %s', (protocol) => {
    ...
    const isSuccessful = responses[protocol]?.status >= 200 && responses[protocol]?.status < 300;
    const skipIfFailed = isSuccessful ? describe : describe.skip;

    skipIfFailed('Basic Response Validation', () => {
    ...
    skipIfFailed('Data Quality Validation', () => {
    ...
    skipIfFailed('Chart Data Validation', () => {
      ...
      it('should have fresh chart data when present', () => {
        ...
        if (data.totalDataChart && data.totalDataChart.length > 0) {
          ...
          expectFreshData(timestamps, 86400 * 5); // 5 days (options may update less frequently)
        }
      });
    ...
    skipIfFailed('Metadata Validation', () => {
  ...
  describe('Edge Cases', () => {
    it('should handle non-existent protocol gracefully', async () => {
      const response = await apiClient.get(
        endpoints.VOLUMES.SUMMARY_OPTIONS('nonexistentprotocol123456')
      );
