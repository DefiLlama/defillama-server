...
import { NewUsersSummaryResponse, isNewUsersSummaryResponse } from './types';
import { newUsersSummaryResponseSchema } from './schemas';
import {
...
describe('New Users API - Summary', () => {
  const testProtocols = ['uniswap', 'pancakeswap', 'blur'];

  const responses: Record<string, ApiResponse<NewUsersSummaryResponse>> = {};

  beforeAll(async () => {
    const results = await Promise.all(
      testProtocols.map((protocol) =>
        apiClient.get<NewUsersSummaryResponse>(endpoints.NEW_USERS_DIM.SUMMARY(protocol))
      )
    );
  ...
  describe.each(testProtocols)('Protocol: %s', (protocol) => {
    describe('Basic Response Validation', () => {
      it('should return successful response with valid structure', () => {
        ...
        expect(isNewUsersSummaryResponse(responses[protocol].data)).toBe(true);
      });

      it('should validate against Zod schema', () => {
        const result = newUsersSummaryResponseSchema.safeParse(responses[protocol].data);
        if (!result.success) {
  ...
  describe('Edge Cases', () => {
    it('should handle non-existent protocol gracefully', async () => {
      const response = await apiClient.get(endpoints.NEW_USERS_DIM.SUMMARY('non-existent-protocol-xyz'));
