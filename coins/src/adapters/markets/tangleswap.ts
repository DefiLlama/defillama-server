...
const graphEndpoint = 'https://shimmer.subgraph.tangleswap.space/subgraphs/name/tangleswap/shimmer'
const query = `{
  tokens(
    orderBy: volumeUSD
    orderDirection: desc
...
async function getPools() {
  const pools = [] as any
  const { data: { data: { tokens } } } = await axios.post(graphEndpoint, { query, operationName: '', variables: {} })
  log('tangleswap', tokens.length)
  tokens.forEach((i: any) => {
    if (i.id === SMR) return;
    const includeToken = i.whitelistPools?.some((i: any) => {
      const smrBal = i.token0.id === SMR ? i.totalValueLockedToken0 : i.totalValueLockedToken1
...
async function getTokenPrices(timestamp: number) {
  ...
  pools.forEach(({ id, derivedETH, decimals, symbol, }: any) => {
    ...
    addToDBWritesList(writes, chain, id, price, decimals, symbol, timestamp, 'tangleswap', 0.9)
  })
  return writes
}
