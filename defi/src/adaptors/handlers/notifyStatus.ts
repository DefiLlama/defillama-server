...
export default async (event: { type: string }) => {
    ...
    if (hasErrors || hasZeroValues) {
        ...
        if (hasErrors)
            await sendDiscordAlert(`${errorsArr.length} adapters failed to update... Retrying... <@!983314132411482143>`, event.type)
        if (hasErrors && errorsArr.length > 35)
            await sendDiscordAlert(`${errorsArr.length} adapters failed to update... Retrying... <@&849669546448388107>`, event.type)
    }
    else {
