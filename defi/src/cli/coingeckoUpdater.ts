...
async function fetchCoinDetails(coinId: string, twitterHandle: string, homepage: string): Promise<string | undefined> {
  try {
    ...
    if (data.links?.twitter_screen_name?.toLowerCase() === twitterHandle.toLowerCase()) {
...
async function updateProtocolsData() {
  ...
  const match = fileContent.match(/const data: Protocol\[\] = (\[[\s\S]*?\]);/);
  if (!match) {
  ...
  for (const protocol of protocols) {
    ...
    for (const coin of matchingCoins) {
      ...
      const coingeckoId = await fetchCoinDetails(coin.id, protocol.twitter, protocol.url);
      if (coingeckoId) {
  ...
  // Update the file with all protocols
  const updatedContent = fileContent.replace(
    /const data: Protocol\[\] = \[[\s\S]*?\];/,
    `const data: Protocol[] = ${JSON.stringify(protocols, null, 2)};`
  );

  try {
    fs.writeFileSync(dataPath, updatedContent);
    console.log('File updated successfully at:', dataPath);
