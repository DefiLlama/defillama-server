...
export const cacheResponseOnR2 = async (key: string, response: string) => {
    ...
    await storeR2JSONString(`${ADAPTORS_FOLDER_KEY}/${key}.json`, response).catch(e => console.error("R2Storage:error:store", key, e.message))
}
...
export const getCachedResponseOnR2 = async <T>(key: string): Promise<ICachedResponse<T> | undefined> => {
    ...
    const objectString = await getR2(`${ADAPTORS_FOLDER_KEY}/${key}.json`).catch(e => console.error("R2Storage:error:get", key, e.message))
