...
import { getBaseTokenUsdcPrices } from "./jupiter";
...
export async function getApiPrices(timestamp: number): Promise<Write[]> {
  ...
  const baseTokenUsdcPrices: { [key: string]: number } =
    await getBaseTokenUsdcPrices();
  ...
  marketInfos.map((marketInfo: MarketInfo) => {
    ...
    const baseTokenPrice = baseTokenUsdcPrices[marketInfo.unit];
