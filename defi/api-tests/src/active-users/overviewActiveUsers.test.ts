...
import { ActiveUsersOverviewResponse, isActiveUsersOverviewResponse } from './types';
import { activeUsersOverviewResponseSchema } from './schemas';
import {
...
describe('Active Users API - Overview', () => {
  let overviewResponse: ApiResponse<ActiveUsersOverviewResponse>;
  let chainResponse: ApiResponse<ActiveUsersOverviewResponse>;

  beforeAll(async () => {
    const [r1, r2] = await Promise.all([
      apiClient.get<ActiveUsersOverviewResponse>(endpoints.ACTIVE_USERS_DIM.OVERVIEW),
      apiClient.get<ActiveUsersOverviewResponse>(endpoints.ACTIVE_USERS_DIM.OVERVIEW_CHAIN('ethereum')),
    ]);
  ...
  describe('All Active Users Overview', () => {
    describe('Basic Response Validation', () => {
      it('should return successful response with valid structure', () => {
        ...
        expect(isActiveUsersOverviewResponse(overviewResponse.data)).toBe(true);
      });

      it('should validate against Zod schema', () => {
        const result = activeUsersOverviewResponseSchema.safeParse(overviewResponse.data);
        if (!result.success) {
  ...
  describe('Chain-specific Active Users Overview', () => {
    describe('Basic Response Validation', () => {
      it('should return successful response with valid structure', () => {
        expectSuccessfulResponse(chainResponse);
        expect(isActiveUsersOverviewResponse(chainResponse.data)).toBe(true);
      });

      it('should validate against Zod schema', () => {
        const result = activeUsersOverviewResponseSchema.safeParse(chainResponse.data);
        if (!result.success) {
          console.log('Validation errors:', JSON.stringify(result.error.issues.slice(0, 10), null, 2));
        }
        expect(result.success).toBe(true);
