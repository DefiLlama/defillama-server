...
function getTokensToExclude(cex: any) {
  ...
  if (cex.coinSymbol) tokensToExclude.push(cex.coinSymbol);

  if (Array.isArray(cex.ownTokens) && cex.ownTokens.length > 0) {
    for (const token of cex.ownTokens) {
      if (!tokensToExclude.includes(token)) tokensToExclude.push(token);
    }
  }
...
if (isTESTMode) console.log(`CEX counts: total=${cexes.length}, withTvl=${cexesWithTvl.length}`);
...
async function addAssetData() {
  ...
  const [tvlData, usdTokensTvl, inflows1d, inflows1w, inflows1m] = await Promise.all([
  ...
  usdTokensTvl.forEach((item: any) => {
    ...
    if (!cex) {
      console.warn(
        `CEX with id ${item.id} not found in protocols data usd tokens tvl: ${cexMetadataIdMap[item.id]?.name}`
      );
      return;
    }
    ...
    if (tokensToExclude.length === 0) {
      ...
      if (isTESTMode)
        console.log(
          `CEX ${cex.name} clean assets tvl: ${sdk.humanizeNumber(cex.cleanAssetsTvl)} (excluded: ${sdk.humanizeNumber(
            excludedTvl
          )})`
        );
    }
...
async function addSpotAndDerivData() {
  ...
  } = (await sdk.coins.getPrices(["coingecko:bitcoin"], "now")) as any;
...
async function run() {
  ...
  if (now - lastUpdateTS < updatePeriodMs && !isTESTMode) {
