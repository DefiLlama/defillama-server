...
const OUT_DIR = path.join(SRC_DIR, "protocols");
...
function isChainCoingeckoIds(node: any): boolean {
  const ann = node?.id?.typeAnnotation?.typeAnnotation;
  return (
    ann?.elementType &&
    ann.elementType?.type === "TSTypeReference" &&
    ann.elementType.typeName?.type === "Identifier" &&
    ann.elementType.typeName.name === "ChainCoinGekcoIds"
  );
}
...
function normalizeChainKey(prop: R.namedTypes.ObjectProperty) {
  let raw = "";
  if (isIdentifier(prop.key)) raw = prop.key.name;
  else if (isStringLiteral(prop.key)) raw = prop.key.value;
  if (!raw) return;

  const norm = getChainDisplayName(raw, false);
  prop.key = toValidKeyNode(norm);
}

function injectDimensionsConfig(obj: R.namedTypes.ObjectExpression) {
  const objectId = (obj.properties.find((p: any) => isObjectProperty(p, "id"))!.value as any).value;
  console.log('objectId', objectId)
  return;
  obj.properties.forEach((p: any) => {
    console.log('property', p)
    if (!isObjectProperty(p, "oraclesBreakdown")) return;
    const arr = p.value;
    if (arr?.type !== "ArrayExpression") return;

    arr.elements?.forEach((oracleEl: any) => {
      if (oracleEl?.type !== "ObjectExpression") return;

      oracleEl.properties.forEach((op: any) => {
        if (!isObjectProperty(op, "chains")) return;
        const chainsArr = op.value;
        if (chainsArr?.type !== "ArrayExpression") return;

        chainsArr.elements?.forEach((chainEl: any) => {
          if (chainEl?.type !== "ObjectExpression") return;

          chainEl.properties.forEach((cp: any) => {
            if (!isObjectProperty(cp, "chain")) return;
            const val = cp.value;
            if (!isStringLiteral(val)) return;

            val.value = getChainDisplayName(val.value, false);
          });
        });
      });
    });
...
function transformChainSource() {
  ...
  visit(ast, {
    visitVariableDeclaration(p) {
      ...
      if (!isChainCoingeckoIds) return false;
      if (isChainCoingeckoIds) {
        const init = isChainCoingeckoIds.init.expression.expression;
        if (init?.type === "ObjectExpression") {
          console.log(`Found chain config object with ${init.properties?.length} properties`);
                ...
                if (chainConfigMap[chainLabel]) {
                  ...
                  prop.value.properties.push(
                    b.objectProperty(
                      ...
                      valueToNode(chainConfigMap[chainLabel])
                    )
                  );
                }
              }
  ...
  console.log('Writing updated chain data to', CHAIN_FILE);
  // writeFileSync(CHAIN_FILE, chainDataOutput.replace(/,\n+/g, ',\n'), "utf8");
...
for (const fileName of FILES) {
  ...
  // writeFileSync(path.join(OUT_DIR, fileName), out, "utf8");
}
