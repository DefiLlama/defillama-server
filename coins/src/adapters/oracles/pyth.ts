...
async function fetchFeedIds(): Promise<Feed[]> {
  const feedsRes = await getConfig(
    "pyth-feeds",
...
export async function pyth(timestamp: number = now) {
  ...
  const res = await api.multiCall({
    target: contract,
    abi: "function getPriceUnsafe(bytes32 id) view returns ((int64 price, uint64 conf, int32 expo, uint256 publishTime) price)",
    calls,
    permitFailure: true,
  });
  const writes: Write[] = [];
  res.forEach((i, idx) => {
    ...
    const { symbol, generic_symbol } = feeds[idx];
    addToDBWritesList(
      writes,
      "pyth",
      generic_symbol,
