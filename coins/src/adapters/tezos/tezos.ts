...
async function getPlentyData() {
  const { data } = await axios.get('https://api.analytics.plenty.network/analytics/tokens')
  const response: any  = []
  data.forEach((i: any) => {
    ...
    response.push({ symbol: token, token: label, decimals, price: +i.price.value, confidence })
  })
