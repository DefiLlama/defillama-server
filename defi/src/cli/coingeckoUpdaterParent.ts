...
async function fetchCoinDetails(coinId: string, twitterHandle: string, homepage: string): Promise<string | undefined> {
  try {
    ...
    // Check if market data exists and no excluded categories
...
async function updateProtocolsData() {
  // Process parent protocols first
  const parentPath = path.join(__dirname, 'parentProtocols.ts');
  console.log('Working with parent protocols file:', parentPath);
  const parentContent = fs.readFileSync(parentPath, 'utf8');
  
  const parentMatch = parentContent.match(/const parentProtocols: IParentProtocol\[\] = (\[[\s\S]*?\]);/);
  if (!parentMatch) {
  ...
  // Update the parent protocols file
  const updatedParentContent = parentContent.replace(
    /const parentProtocols: IParentProtocol\[\] = \[[\s\S]*?\];/,
    `const parentProtocols: IParentProtocol[] = ${JSON.stringify(parentProtocols, null, 2)};`
  );

  try {
    fs.writeFileSync(parentPath, updatedParentContent);
    console.log('Parent protocols file updated successfully');
