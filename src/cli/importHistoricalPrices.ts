...
async function main() {
    if ((toTimestamp - toTimestamp) > (90 * DAY)) {
        throw new Error("Timestamp difference is higher than 90 days, it needs to be lower in order to get hourly rates")
    }
    setTimer(1000);
    const coins = (await coingeckoRequest('https://api.coingecko.com/api/v3/coins/list')) as Coin[]
    for (let coinIndex = startingCoinIndex; coinIndex < coins.length; coinIndex++) {
        const coin = coins[coinIndex];
        console.log(`Getting data for ${coin.id} at index ${coinIndex}...`)
        const [coinData, { prices }] = await Promise.all([
            coingeckoRequest(`https://api.coingecko.com/api/v3/coins/${coin.id}?localization=false&tickers=false&market_data=false&community_data=false&developer_data=false&sparkline=false`),
            coingeckoRequest(`https://api.coingecko.com/api/v3/coins/${coin.id}/market_chart/range?vs_currency=usd&from=${fromTimestamp}&to=${toTimestamp}`) as PriceRangeResponse
        ]);
