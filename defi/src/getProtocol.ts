...
export async function craftProtocolResponse(rawProtocolName:string|undefined, useNewChainNames: boolean, useHourlyData: boolean){
  ...
  response.otherProtocols = (protocolData.symbol === "SUSHI" ||protocolData.symbol === "SUN" || protocolData.symbol === "AAVE") ? protocols.filter(p => p.symbol === protocolData.symbol)?.map(p => p.name) : []
