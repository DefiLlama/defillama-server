...
export async function storeGetCharts({ ...options }: any = {}) {
  ...
  await Promise.all(
    Object.entries(sumDailyTvls).map(async ([chain, chainDailyTvls]) => {
      ...
      let filename = chain === "total" ? "lite/charts-total" : `lite/charts/${chain}`;

      const data = roundNumbersInObject(chainResponse)

      if (data?.tvl) {
        const simpleChartData = data.tvl.map(([date, totalLiquidityUSD]: any) => ({ date, totalLiquidityUSD }))
      ...
      await storeRouteData(filename, data);
    })
  );
