...
async function getCoreAssetsFile() {
  if (!coreAssetFile)
    coreAssetFile = axios.get('https://raw.githubusercontent.com/DefiLlama/DefiLlama-Adapters/main/projects/helper/coreAssets.json').then((res: any) => res.data)
...
async function getCoreTokenSet(chain: string) {
  if (!coreAssetsCache[chain]) {
    ...
    const tokens = Object.values(coreAssets[chain] ?? {}).map((i: any) => i.toLowerCase())
...
export function getUniV2Adapter({ endpoint, project, chain, minLiquidity = MinLiquidity, minVolume = MinVolume, factory, uniqueLPNames = false, getReservesAbi = defaultGetReservesAbi,
  ...
  const graphAdapter = async (timestamp: number = 0) => {
    ...
    allData.forEach((pair: any) => {
      ...
      if (confidence > 0.8)
        addToDBWritesList(writes, chain, pair.id, price, decimals, symbol, timestamp, project, confidence)
  ...
  const onChainAdapter = async (timestamp: number = 0) => {
    ...
    const lpSupplies = await api.multiCall({ abi: 'erc20:totalSupply', calls: pairs, permitFailure: true, })
    const reserves = await api.multiCall({ abi: getReservesAbi, calls: pairs, permitFailure: true, })
    ...
    pairs.forEach((pair: any, idx: number) => {
      ...
      } else if (coinsDataMap[token1]) { // if we know the price of token1
    ...
    Object.entries(tokenData).forEach(([tokenId, { supply, liquidity }]: any) => {
      const confidence = calculateConfidence(liquidity, minLiquidity / 2)
      ...
      if (confidence > 0.8)
        addToDBWritesList(writes, chain, tokenId, price, decimals, symbol, timestamp, project, confidence)
    })
  ...
  return async (timestamp: number = 0) => {
    if (endpoint) {
      try {
        ...
        if (factory)
          console.error('Error using graph adapter' + (e as any).toString() + endpoint)
        else
          throw e
      }
    }
